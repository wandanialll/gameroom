...
let viewX = 0, viewY = 0; // pan offset
let scale = 1;
const MIN_SCALE = 0.2;
const MAX_SCALE = 3;
...
function connectSocket() {
  ...
  socket.on('game_state', (state) => {
    ...
    if (state.complete && state.completedImage) {
      showCompletion({ completedImage: state.completedImage, players, duration: null });
    }
  });
  ...
  socket.on('piece_grabbed', ({ pieceId, heldBy, color }) => {
    const p = getPiece(pieceId);
    if (p) { p.heldBy = heldBy; p._heldColor = color; }
  });

  socket.on('piece_moved', ({ pieceId, x, y }) => {
    const p = getPiece(pieceId);
    if (p && p.heldBy !== myId) { p.x = x; p.y = y; }
  });

  socket.on('piece_dropped', ({ pieceId, x, y, locked, lockedBy, color }) => {
    const p = getPiece(pieceId);
    if (!p) return;
    p.x = x; p.y = y;
    p.heldBy = null;
  ...
  socket.on('piece_released', ({ pieceId }) => {
    const p = getPiece(pieceId);
    if (p) { p.heldBy = null; p._heldColor = null; }
  });
...
function getPieceAt(wx, wy) {
  ...
  const { pieceW, pieceH } = gameState;
  // Top-most first (held > free > locked)
  const order = [
    ...pieces.filter(p => p.heldBy === myId),
...
function onMouseMove(e) {
  ...
  if (heldPieceId !== null) {
    ...
    if (p) {
      p.x = wx - dragOffsetX;
      p.y = wy - dragOffsetY;
      socket.emit('piece_move', { pieceId: p.id, x: p.x, y: p.y });
      markDirty();
    }
  } else if (isPanning) {
...
function onMouseUp(e) {
  if (heldPieceId !== null) {
    ...
    if (p) {
      ...
      p.x = nx; p.y = ny;
      p.heldBy = null;
      socket.emit('piece_drop', { pieceId: p.id, x: nx, y: ny });
    }
    heldPieceId = null;
...
function updatePlayers() {
  ...
  list.innerHTML = '';
  Object.values(players).slice(0, 8).forEach(p => {
